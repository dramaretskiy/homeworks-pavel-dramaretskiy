...
function deleteTodoFromView(id) {
  ...
  alertMessage("Todo was deleted", (className = "alert-info"));
...
function todoTemplate({ id, title, text }) {
  return `
    <tr data-id="${id}">
        <td>${title}</td>
        <td>${text}</td>
...
function alertMessage(message, className = "") {
  removeAlert();
  let template = alertTemplate(message, className);
...
function removeAlert() {
  let curretAlert = document.querySelector(".alert");
